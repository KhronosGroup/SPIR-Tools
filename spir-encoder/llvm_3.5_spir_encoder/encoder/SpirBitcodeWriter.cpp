//===------------- SpirBitcodeWriter.cpp - Bitcode Writer ----------------===//
//
//                              SPIR Tools
//
...
static void WriteModuleInfo(const Module *M, const ValueEnumerator &VE,
  ...
  if (!M->getDataLayoutStr().empty())
  {
    ...
    {
      // Ensure that data layout is valid for SPIR
      bool Is64Bit = (M->getTargetTriple().compare(0, 6, "spir64") == 0);
      const char *SpirDL = Is64Bit ? SPIR64_DATALAYOUT : SPIR32_DATALAYOUT;
      assert(*M->getDataLayout() == DataLayout(SpirDL) &&
            "Data layout not valid for SPIR");

      // Output data layout using LLVM 3.2 string representation
      WriteStringRecord(bitc::MODULE_CODE_DATALAYOUT, SpirDL,
                        0/*TODO*/, Stream);
    }
    else
    {
...
namespace SPIR
{
  ...
  /// stream.
  void WriteBitcodeToFile_SPIR(const Module *M, raw_ostream &Out) {
